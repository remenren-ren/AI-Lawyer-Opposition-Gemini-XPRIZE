...
OFFLINE_APP = HERE / "Nido_StrikeOver_Offline_EN.py"
ONLINE_APP = HERE / "Nido_StrikeOver_Online_EN.py"
PINCH_APP = HERE / "pinch_payment_gui.py"
CLIENT_RECEPTION_APP = HERE / "ai_law_firm_reception_launcher.py"
...
class StrikeOverLauncherEN:
    ...
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("AI Lawyer Opposition - Pinch Competition Edition")
        self.root.geometry("1120x720")
        self.root.minsize(980, 640)
        self.root.configure(bg=self.C["bg"])
        self.cloud_backend = get_backend()
        self.status_var = tk.StringVar(value="Ready: sign in can restore provider settings when admin cloud config is supplied.")
        self.root.withdraw()
        self.build_ui()
        self.restore_existing_cloud_session()
        self.root.deiconify()
    ...
    def build_ui(self):
        ...
        tk.Label(
            title,
            text="AI Lawyer Opposition - Pinch Competition Edition",
            bg=self.C["bg"],
            fg=self.C["gold"],
        ...
        tk.Label(
            title,
            text="Choose a version; API and frame settings live inside the app",
            bg=self.C["bg"],
            fg=self.C["muted"],
        ...
        self.build_version_card(
            ...
            0,
            "Offline Version",
            "Local weakness scan / tactic package / confidential matters",
            self.C["offline"],
            [
                "Case content is not uploaded by default.",
                "Best for important, sensitive, or large-case preprocessing.",
                "Model providers are configured inside the offline version.",
            ],
            "Offline Version",
            lambda: self.launch_app(OFFLINE_APP, "Offline Version"),
        )
        self.build_version_card(
            cards,
            1,
            "Online Version",
            "Full model-assisted opposition output / low-sensitivity matters",
            self.C["online"],
            [
                "Keeps the legacy full opposition workflow.",
                "Suitable for low-sensitivity matters and comparison testing.",
                "Online model settings stay separate from the offline version.",
            ],
            "Online Version",
            lambda: self.launch_app(ONLINE_APP, "Online Version"),
        )
        ...
        self.make_button(
            ...
            lambda: self.launch_app(PINCH_APP, "Pinch Payments Sandbox"),
            self.C["pinch"],
            padx=12,
            pady=5,
        ...
        self.make_button(
            bottom,
            "AI Client Reception",
            lambda: self.launch_app(CLIENT_RECEPTION_APP, "AI Client Reception"),
            "#0f766e",
            padx=12,
            pady=5,
        ).pack(side=tk.RIGHT, padx=(0, 8))
        self.make_button(
            bottom, "Google Cloud", self.open_cloud_integration_center, "#b7791f", padx=12, pady=5
        ).pack(side=tk.RIGHT, padx=(0, 8))
    ...
    def build_cloud_panel(self):
        ...
        tk.Label(
            panel,
            text="Google Cloud Account and Provider Sync",
            bg=self.C["panel"],
            fg=self.C["text"],
        ...
        tk.Label(
            panel,
            text="Firebase Authentication identifies the user; Firestore can restore that user's provider settings. Local demo access remains available for competition review.",
            bg=self.C["panel"],
            fg=self.C["muted"],
        ...
        self.cloud_status_var = tk.StringVar(value="Optional. Local use does not require sign-in. Sign-in restores settings only; it does not connect models.")
        ...
        self.cloud_google_button = self.make_button(
            actions, "Sign in with Google", self.sign_in_with_google, "#f8fafc", fg="#111827", padx=12, pady=5
        )
        self.cloud_google_button.pack(side=tk.RIGHT, padx=3)
        self.cloud_save_api_button = self.make_button(
            actions, "Save APIs to Google", self.save_provider_config_to_cloud, "#0f766e", padx=12, pady=5
        )
        self.cloud_save_api_button.configure(state=tk.DISABLED, bg="#4b5563")
        self.cloud_save_api_button.pack(side=tk.RIGHT, padx=3)
        self.cloud_signout_button = self.make_button(
            actions, "Sign out", self.sign_out, "#5a2430", padx=12, pady=5
        )
        self.cloud_signout_button.configure(state=tk.DISABLED, bg="#4b5563")
        self.cloud_signout_button.pack(side=tk.RIGHT, padx=3)
    ...
    def set_cloud_signed_in(self, message):
        ...
        self.cloud_signout_button.configure(state=tk.NORMAL, bg="#5a2430")
        session = self.cloud_backend.load_session(refresh=False)
        can_sync = bool(session.get("uid") and session.get("id_token"))
        self.cloud_save_api_button.configure(
            state=tk.NORMAL if can_sync else tk.DISABLED,
            bg="#0f766e" if can_sync else "#4b5563",
        )
    ...
    def set_cloud_sign_in_failed(self, message):
        ...
        self.cloud_google_button.configure(text="Sign in with Google", state=tk.NORMAL, bg="#f8fafc", fg="#111827")
        self.cloud_save_api_button.configure(state=tk.DISABLED, bg="#4b5563")
        self.cloud_signout_button.configure(state=tk.DISABLED, bg="#4b5563")

    def restore_existing_cloud_session(self):
        try:
            session = self.cloud_backend.load_session(refresh=True)
        except Exception:
            return
        if session.get("signed_in") and session.get("uid") and session.get("id_token"):
            self.set_cloud_signed_in(
                f"Google account ready: {session.get('account') or session.get('uid')}. Saved APIs can be restored or updated."
            )
    ...
    def sign_out(self):
        ...
        self.cloud_save_api_button.configure(state=tk.DISABLED, bg="#4b5563")
        self.cloud_signout_button.configure(state=tk.DISABLED, bg="#4b5563")
        self.set_cloud_status("Signed out. Local API files remain on this computer.", self.C["gold"])

    def save_provider_config_to_cloud(self):
        session = self.cloud_backend.load_session(refresh=True)
        if not session.get("uid") or not session.get("id_token"):
            messagebox.showinfo("Save APIs to Google", "Sign in with Google before saving API settings.")
            return
        if not PROVIDER_CONFIG.exists():
            messagebox.showinfo("Save APIs to Google", "No local API provider configuration was found.")
            return
        try:
            data = json.loads(PROVIDER_CONFIG.read_text(encoding="utf-8-sig"))
        except Exception as exc:
            messagebox.showerror("Save APIs to Google", f"The local API configuration is invalid:\n{str(exc)[:240]}")
            return
        providers = list(data.get("providers") or [])
        if not providers or not any(str(row.get("key") or "").strip() for row in providers):
            messagebox.showinfo("Save APIs to Google", "No API keys are currently saved in the provider list.")
            return
        if not messagebox.askyesno(
            "Save APIs to Google",
            "Save the current provider names, model settings, and API keys to your private Google account document?\n\n"
            "Access is restricted to your signed-in Firebase user. Restored providers remain disabled until you press Verify.",
        ):
            return
        safe_copy = dict(data)
        safe_rows = []
        ...
        safe_copy["providers"] = safe_rows
        self.cloud_save_api_button.configure(text="Saving...", state=tk.DISABLED, bg="#4b5563")
        self.set_cloud_status("Saving API provider settings to your Google account...", "#93c5fd")

        def work():
            try:
                saved = self.cloud_backend.sync_provider_profile(safe_copy)
                if not saved:
                    raise RuntimeError("Firebase did not accept the provider profile.")
                self.root.after(0, lambda: (
                    self.cloud_save_api_button.configure(text="Save APIs to Google", state=tk.NORMAL, bg="#0f766e"),
                    self.set_cloud_status("API provider settings saved to your Google account. Restored models will require Verify.", "#86efac"),
                ))
            except Exception as exc:
                error_text = str(exc)
                self.root.after(0, lambda text=error_text: (
                    self.cloud_save_api_button.configure(text="Save APIs to Google", state=tk.NORMAL, bg="#0f766e"),
                    self.set_cloud_status("Cloud API save failed.", "#f87171"),
                    messagebox.showerror("Save APIs to Google", text[:500]),
                ))
    ...
    def write_cloud_session(self, account, source):
        data = {
            ...
            "note": "Session gate only. Provider rows still require Verify inside Online or Offline.",
        }
    ...
    def load_admin_cloud_config(self):
        if not ADMIN_CLOUD_CONFIG.exists():
            return None
        try:
            return json.loads(ADMIN_CLOUD_CONFIG.read_text(encoding="utf-8"))
        except Exception as exc:
    ...
    def load_cloud_integration_config(self):
        if not CLOUD_INTEGRATION_CONFIG.exists():
            return {}
        try:
            return json.loads(CLOUD_INTEGRATION_CONFIG.read_text(encoding="utf-8-sig"))
        except Exception as exc:
            raise RuntimeError(f"Google Cloud integration config is invalid: {exc}") from exc

    def save_cloud_integration_config(self, data):
        CLOUD_INTEGRATION_CONFIG.write_text(json.dumps(data, ensure_ascii=False, indent=2), encoding="utf-8")
        ...
        for row in data.get("providers", []) or []:
            if str(row.get("name") or "").strip().lower() == "gemini":
                if str(row.get("base_url") or "").strip() and str(row.get("model") or "").strip():
                    return True, "Configured locally; the model still requires Verify inside the selected version."
    ...
    def cloud_service_rows(self):
        ...
        run = config.get("cloud_run") or {}
        run_url = str(run.get("service_url") or "").strip()
        run_active = bool(run.get("enabled") and run_url and "YOUR_CLOUD_RUN_SERVICE" not in run_url)
        firestore = config.get("firestore") or {}
        firebase = config.get("firebase") or {}
        project_id = str(config.get("project_id") or admin.get("project_id") or "").strip()
        firestore_active = bool(firestore.get("enabled") and project_id and run_active)
        firebase_active = bool(
            firebase.get("enabled")
            and project_id
        ...
        return [
            ...
            ("Firebase Authentication", firebase_active, "Google and email/password authentication are configured." if firebase_active else "Code is installed; connect a Firebase web app to activate Google sign-in."),
            ("Cloud Firestore", firestore_active, "Cloud Run backend storage and provider-profile restore are active." if firestore_active else "Restore code is installed; no deployed Firestore backend is configured."),
            ("Cloud Run", run_active, f"Configured endpoint: {run_url}" if run_active else "Deployable service is included; no deployed run.app endpoint is configured."),
            ("Cloud Storage", bool(storage.get("enabled") and str(storage.get("bucket") or "").strip()), f"Authorized report bucket: {storage.get('bucket')}" if storage.get("enabled") and str(storage.get("bucket") or "").strip() else "Prepared for authorized reports and backups; no bucket is configured."),
            ("Cloud Logging", bool((config.get("cloud_logging") or {}).get("enabled") and run_active), "Sanitized operational logging is active." if run_active else "Prepared; activates with Cloud Run."),
            ("Secret Manager", bool((config.get("secret_manager") or {}).get("enabled") and run_active), "Server-side secrets route is active." if run_active else "Prepared for server secrets; local model keys are not uploaded by this panel."),
            ("Vertex AI", bool((config.get("vertex_ai") or {}).get("enabled") and run_active), f"Managed review model: {(config.get('vertex_ai') or {}).get('model') or 'gemini-2.5-flash'}" if run_active else "Managed Gemini review route is installed; activates after deployment."),
        ]
    ...
    def open_cloud_integration_center(self):
        ...
        tk.Label(header, text="Google Cloud Integration Center", bg="#111827", fg=self.C["gold"], font=("Microsoft YaHei UI", 18, "bold")).pack(anchor="w")
        tk.Label(header, text="Real configuration state only. Installed code is not marked active until a project, credential, bucket, or deployed endpoint exists.", bg="#111827", fg="#cbd5e1", font=("Microsoft YaHei UI", 10), wraplength=850, justify=tk.LEFT).pack(anchor="w", pady=(5, 0))
        endpoint = tk.Frame(dialog, bg="#1f2937", padx=16, pady=12)
        endpoint.pack(fill=tk.X, padx=22, pady=(0, 12))
        tk.Label(endpoint, text="Cloud Run service URL", bg="#1f2937", fg="#e5e7eb", font=("Microsoft YaHei UI", 10, "bold")).pack(anchor="w")
        run_url_var = tk.StringVar(value=str((config.get("cloud_run") or {}).get("service_url") or ""))
        tk.Entry(endpoint, textvariable=run_url_var, bg="#111827", fg="#f8fafc", insertbackground="#f8fafc", relief=tk.FLAT, bd=7, font=("Consolas", 10)).pack(fill=tk.X, pady=(6, 0))
        shell = tk.Frame(dialog, bg="#111827")
        shell.pack(fill=tk.BOTH, expand=True, padx=22)
        canvas = tk.Canvas(shell, bg="#111827", highlightthickness=0)
        scroll = tk.Scrollbar(shell, orient=tk.VERTICAL, command=canvas.yview)
        ...
        def render_rows():
            ...
            for name, active, note in self.cloud_service_rows():
                row = tk.Frame(inner, bg="#1f2937", padx=14, pady=12, highlightthickness=1, highlightbackground="#374151")
                row.pack(fill=tk.X, pady=(0, 7))
                tk.Label(row, text="ACTIVE" if active else "NOT ACTIVE", bg="#065f46" if active else "#4b5563", fg="white", width=12, pady=5, font=("Microsoft YaHei UI", 9, "bold")).pack(side=tk.LEFT, padx=(0, 12))
                body = tk.Frame(row, bg="#1f2937")
                body.pack(side=tk.LEFT, fill=tk.X, expand=True)
                tk.Label(body, text=name, bg="#1f2937", fg="#f8fafc", font=("Microsoft YaHei UI", 11, "bold")).pack(anchor="w")
                tk.Label(body, text=note, bg="#1f2937", fg="#cbd5e1", font=("Microsoft YaHei UI", 9), wraplength=680, justify=tk.LEFT).pack(anchor="w", pady=(2, 0))
        ...
        def test_cloud_run():
            ...
            def work():
                try:
                    ...
                    self.root.after(0, lambda: messagebox.showinfo("Cloud Run", "Cloud Run health check succeeded."))
                except Exception as exc:
                    text = str(exc)
                    self.root.after(0, lambda: messagebox.showerror("Cloud Run", f"Health check failed:\n{text}"))
            threading.Thread(target=work, daemon=True).start()

        def deploy_cloud_run():
            if not CLOUD_DEPLOY_SCRIPT.exists():
                messagebox.showerror("Cloud Run", f"Deployment script was not found:\n{CLOUD_DEPLOY_SCRIPT}")
                return
            subprocess.Popen(["cmd.exe", "/k", str(CLOUD_DEPLOY_SCRIPT)], cwd=str(CLOUD_SERVICE_DIR), creationflags=getattr(subprocess, "CREATE_NEW_CONSOLE", 0))
            messagebox.showinfo("Cloud Run Deployment", "The deployment console is open. Sign in to Google Cloud, select a billing-enabled project, and keep the console open until it prints the deployed run.app URL.")
        ...
        self.make_button(actions, "Deploy Cloud Run", deploy_cloud_run, "#b7791f", padx=14, pady=7).pack(side=tk.LEFT)
        self.make_button(actions, "Open Cloud Service", lambda: os.startfile(str(CLOUD_SERVICE_DIR)), "#374151", padx=14, pady=7).pack(side=tk.LEFT, padx=8)
    ...
    def sign_in(self):
        ...
        if not all([account, password]):
            ...
            return
        # Keep the competition demo path local even when the real Firebase
        # configuration is installed for the recorded Google sign-in demo.
        if account.lower() == "demo" and password == "demo":
            self.write_cloud_session(account, "local-demo")
            self.set_cloud_signed_in("Demo sign-in accepted locally. Launch Online or Offline; verify provider rows before model use.")
            ...
            try:
                ...
                def done():
                    ...
                    if restored:
                        ...
                        self.set_cloud_signed_in("Provider settings restored. Open Online or Offline and click Verify before use.")
                        ...
                        self.status_var.set("Sign-in OK. Configure providers inside Online or Offline.")
    ...
    def sign_in_with_google(self):
        ...
        if not config or not all(required.values()):
            messagebox.showinfo(
                ...
                "Google sign-in is installed but is not active yet.\n\n"
                "Connect a Firebase web application in admin_cloud_config.local.json "
                "to activate it. Competition reviewers can use demo / demo without "
                "creating an account.",
            )
            return
        if not FIREBASE_GOOGLE_LOGIN_BRIDGE.exists():
            messagebox.showerror("Google Sign-In", f"Google sign-in bridge not found:\n{FIREBASE_GOOGLE_LOGIN_BRIDGE}")
            return
        ...
        def work():
            try:
                ...
                completed = subprocess.run(
                    [str(python_exe), str(FIREBASE_GOOGLE_LOGIN_BRIDGE), "--config", str(ADMIN_CLOUD_CONFIG)],
                    cwd=str(HERE),
                    capture_output=True,
                    text=True,
                    encoding="utf-8",
                ...
                def done():
                    self.cloud_backend.store_firebase_session(
                        user_id, email or user_id, token, refresh_token, result.get("expires_in") or 3600
                    )
                    self.cloud_backend.run_async(
                        self.cloud_backend.record_event, "firebase_google_sign_in", {"source": "dual_launcher"}
                    )
                    if restored:
                        PROVIDER_CONFIG.write_text(json.dumps(restored, ensure_ascii=False, indent=2), encoding="utf-8")
                        self.set_cloud_signed_in(
                        ...
                        self.status_var.set("Firebase identity verified. Configure providers inside Online or Offline.")
    ...
    def restore_provider_config_from_token(self, user_id, token, config):
        ...
        # Firebase Authentication is usable without Firestore. A login-only
        # competition build can deliberately leave Firestore disabled, which
        # makes the optional provider-profile restore return HTTP 403. Keep the
        # verified Google identity and continue without restored provider rows.
        if doc.status_code in (403, 404):
            return None
    ...
    def launch_app(self, script_path, label):
        if not script_path.exists():
            ...
            return
        try:
            subprocess.Popen([self.find_pythonw(prefer_dnd=(script_path in (ONLINE_APP, OFFLINE_APP))), str(script_path)], cwd=str(HERE))
    ...
    def show_startup_disclaimer(self):
        ...
        notices = [
            ...
            ("Confidential matters", "The Online Version may send case content to configured model providers after API verification. Use Offline or redact sensitive information when confidentiality is critical."),
        ]
