...
def show_main_opposition_mode_dialog(parent, standard_command, advanced_command):
    win = tk.Toplevel(parent)
    win.title("Choose Main Opposition Depth")
    ...
    tk.Label(shell, text="Choose Main Opposition Depth", bg="#0f1724", fg="#f4cf61",
    ...
    add_card(
        "Standard Opposition",
        "The established multi-provider opposition workflow.",
        ["Runs the existing opposition, rebuttal, and review route.",
         "Preserves the current standard workflow without changes."],
        "#52c7bd", standard_command, True,
    )
    add_card(
        "Advanced Main Opposition 2R",
        "18 independent full-case attacks, followed by 18 independent full-case responses.",
        ["Every dimension rereads the complete matter in both rounds.",
         "Round 2 rereads the matter and receives every Round-1 attack.",
...
def build_main_opposition_report(output_dir, case_name, records):
    sections = []
    for record in records:
        ...
        <section id="d{record['number']:02d}">
          ...
          <div class="rounds"><article><h3>Round 1 - Negative-Side Attack</h3><pre>{attack}</pre></article>
          <article><h3>Round 2 - Positive-Side Response</h3><pre>{response}</pre></article></div>
        </section>""")
    nav = "".join(
        f'<a href="#d{record["number"]:02d}">{record["number"]:02d} {html.escape(record["dimension"])}</a>'
        for record in records
    )
    document = f"""<!doctype html><html><head><meta charset="utf-8"><title>{html.escape(case_name)} - Advanced Main Opposition 2R</title>
    <style>
    ...
    </style></head><body><header><h1>Advanced Main Opposition 2R Review</h1>
    <p><b>Matter:</b> {html.escape(case_name)}</p>
    <p class="notice">Round 1 contains 18 independent full-matter attacks by negative-side counsel. Round 2 contains 18 independent positive-side rereadings of the complete matter and the complete Round-1 attack dossier. This is lawyer-preparation reference material, not a final legal conclusion.</p></header>
    <nav>{nav}</nav><main>{''.join(sections)}</main></body></html>"""
    path = Path(output_dir) / "00_COMPLETE_MAIN_OPPOSITION_2R.html"
    path.write_text(document, encoding="utf-8")
    return path


class AdvancedMainOpposition2R:
    C = {"bg": "#0c1421", "panel": "#172235", "line": "#304158", "text": "#eef4fb",
         "muted": "#a9b9cb", "gold": "#f4cf61", "teal": "#49d2c5", "red": "#d75870"}

    def __init__(self, parent, matter_context, provider_routes, case_name="Matter", privacy_label="", model_caller=None):
        self.parent = parent
        ...
        self.win.title("Advanced Main Opposition 2R Review")
    ...
    def build_ui(self):
        ...
        tk.Label(header, text="Advanced Main Opposition 2R", bg=self.C["bg"], fg=self.C["gold"],
                 font=("Segoe UI", 19, "bold")).pack(anchor="w")
        tk.Label(header, text="18 negative-side full-matter attacks, then 18 positive-side full-matter responses informed by every attack",
        ...
        top.pack(fill=tk.X)
        tk.Label(top, text="Round 1: Negative side attacks  |  Round 2: Positive side responds",
                 bg=self.C["panel"], fg=self.C["text"], font=("Segoe UI", 10, "bold")).pack(side=tk.LEFT)
        ...
        tk.Label(entry, text="Each dimension receives the complete prepared matter independently. Round 2 also receives the complete Round-1 attack dossier.",
                 bg=self.C["panel"], fg=self.C["muted"], font=("Segoe UI", 10),
                 wraplength=1000, justify=tk.LEFT).pack(anchor="w", pady=(10, 2))
        ...
        self.start_btn = tk.Button(footer, text="Start Advanced Main Opposition 2R", command=self.start,
    ...
    def _call_dimension(self, prompt, dimension_number, round_number):
        ...
        for index, route in enumerate(routes):
            ...
            try:
                personality_idx = dimension_number + round_number + index
                if callable(self.model_caller):
                    ...
                    client = LLMClient(provider, key, personality_idx=personality_idx)
                    result = client.chat_text(prompt, temperature=0.68, max_tokens=4200)
                if not str(result or "").strip():
                    raise RuntimeError("model returned an empty response")
                return str(result).strip(), provider, errors
    ...
    def run(self):
        ...
        self.output_dir = HERE / "runs" / f"advanced_main_opposition_2r_{timestamp}"
        ...
        for number, (dimension_cn, _desc_cn) in enumerate(ALL_DIMENSIONS, 1):
            ...
            self.ui(number - 1, f"Round 1: Negative side attack ({number}/18)", self.C["gold"])
            prompt = f"""Act as senior counsel for the Negative side. Read the entire matter below again from the perspective of {dimension} ({description}).

Develop the strongest attack this dimension supports against the Positive side's case as a whole. Use any argument, evidence, omission, chronology, inconsistency, legal element, alternative explanation, procedural move, or practical litigation angle in the complete record. Explore unusual but defensible angles where the record supports them. Distinguish what the record proves from what is only inferred, and confront the strongest version of the Positive side rather than an easy caricature.

Write a natural, forceful litigation memorandum for a lawyer. Do not return JSON. Do not follow a fixed heading template. Do not mechanically fill fields. Explain why the attack matters and include concrete questions, evidentiary demands, or litigation moves where useful. If this dimension does not support a strong attack, say so briefly and explain the limit instead of inventing one.
...
{self.matter_context}"""
        ...
        (self.output_dir / "00_R1_NEGATIVE_ATTACK_DOSSIER.md").write_text(dossier, encoding="utf-8")
        ...
        if not self.cancel_event.is_set() and len(records) == 18:
            for index, record in enumerate(records, 1):
                ...
                description = DIMENSION_DESC_EN.get(dimension_cn, "Independent whole-matter legal review.")
                self.ui(index - 1, f"Round 2: Positive side rereading ({index}/18)", "#7fb3ff")
                prompt = f"""Act as senior counsel for the Positive side. Reread the entire matter below from the perspective of {dimension} ({description}). Then read every Round-1 attack in the complete Negative-side attack dossier.

Respond to the Negative side's attack campaign using the complete case, not an isolated sentence. Do not answer only the same-dimension attack: use the entire Round-1 dossier and every relevant fact, argument, and evidence item. Address the strongest attacks rather than easy ones. Defend, qualify, narrow, reformulate, or where strategically necessary partially concede a point. You may expose contradictions between attacks, rely on other evidence, shift emphasis, identify missing proof, and counterattack where professionally justified.
...
{dossier}"""
        ...
        for record in records:
            ...
            professional_findings.append({
                ...
                "title": f"{record.get('dimension')} whole-matter attack",
                "finding": record.get("attack"),
                "affected_side": "Positive side",
                "factual_basis": "See complete prepared matter and dimension memorandum",
                ...
                "model": "Not recorded",
                "source_reference": f"Round 1 / dimension {number}",
                "review_status": "ai_generated_unverified",
            })
            matrix.append({
                "issue_id": f"AOP-{number:03d}",
                "dimension": record.get("dimension"),
        ...
        professional_report = build_standard_report(
            "advanced_main_opposition_2r",
            "advanced_whole_matter_two_round_opposition",
            self.case_name,
            ...
            limitations=[
                "Advocacy outputs intentionally test opposing positions and are not neutral legal conclusions.",
                "A strong attack or response does not establish factual truth, admissibility, governing law or likely outcome.",
            ],
        )
        write_standard_companions(self.output_dir, "advanced-main-opposition-2r", professional_report)
        try:
            self.report_path = build_main_opposition_report(self.output_dir, self.case_name, records)
        except Exception as exc:
            self.report_path = None
    ...
    def finish(self):
        ...
        self.start_btn.config(text="Start New Advanced Main Opposition 2R", bg="#f2c94c",
...
def open_advanced_main_opposition_review(
    parent,
    matter_context,
    provider_routes,
    case_name="Matter",
    ...
    return AdvancedMainOpposition2R(
        parent,
        matter_context,
        provider_routes,
        case_name,
